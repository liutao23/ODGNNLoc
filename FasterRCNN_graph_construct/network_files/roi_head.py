...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        all_scores = []
        all_labels = []
        all_features = []  # 新增一个列表，用于保存筛选后的特征
        # 遍历每张图像预测信息
        for boxes, scores, image_shape,features in zip(pred_boxes_list, pred_scores_list, image_shapes, pred_features_list):
            ...
            # 对features的维度进行扩展，方便后续筛选
            features = features.unsqueeze(1).expand(-1, num_classes, -1)
    ...
    def forward(self,
        ...
        if self.training:
            ...
            boxes, scores, labels, box_feature = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes, box_features)
